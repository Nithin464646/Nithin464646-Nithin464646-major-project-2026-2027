...
import { 
  ...
  RefreshCw, TrendingUp, TrendingDown, BookOpen, BrainCircuit
} from "lucide-react";
import { 
...
export default function FuturePricePredictor({ userProfile }: FuturePricePredictorProps) {
  // Input form states
  ...
  // Run automatically on load if nothing has been loaded
  useEffect(() => {
    if (!prediction && !isSimulating && selectedMarket) {
      ...
      <div className="p-6 rounded-3xl border border-[#d1e4d5] bg-gradient-to-br from-[#edf4ee] to-[#d1e4d5] relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            <span className="text-[10px] font-bold text-[#1a5c38] uppercase tracking-widest flex items-center gap-1">
              <Sparkles className="w-3.5 h-3.5 text-[#1a5c38]" /> Advanced Machine Learning Module
            </span>
            <h2 className="text-2xl font-bold tracking-tight text-[#1a2e1c]">
              Dynamic Crop <span className="text-[#1a5c38]">Future Price Forecaster</span>
            </h2>
            <p className="text-sm text-[#4a6550] max-w-2xl leading-relaxed">
              Plan your sowing cycles and harvest dates by simulating custom agricultural weather trends, pest thresholds, and soil organic quality variables. Runs automated regression modeling synced with local Karnataka APMC rates.
            </p>
          </div>
          ...
          <button
            ...
            className="md:self-center px-4 py-2.5 bg-[#1a5c38] text-white font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all hover:bg-[#134429] disabled:opacity-50 cursor-pointer shadow-sm"
          >
            <BrainCircuit className={`w-4 h-4 ${isSimulating ? "animate-spin" : ""}`} />
            {isSimulating ? "Simulating..." : "Run Price Simulation"}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <section id="simulation-control-card" className="lg:col-span-4 p-5 rounded-2xl border border-[#d1e4d5] bg-white space-y-5">
          <div className="border-b border-[#e8f2e9] pb-3">
            <h3 className="text-xs font-bold uppercase tracking-wider text-[#1a5c38]">
          ...
          <div className="space-y-4">
            ...
            {/* Crop Select */}
            <div>
              <label className="text-xs font-semibold text-[#4a6550] block mb-1.5">
                Target Crop
              </label>
              <select
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-1.5 p-1 rounded-xl bg-[#f4f8f4] border border-[#d1e4d5]">
                {[
                  ...
                  <button
                    ...
                    type="button"
                    className={`text-xs py-1.5 rounded-lg border transition-all cursor-pointer ${timeframe === opt.value ? "bg-[#1a5c38] border-[#1a5c38] text-white font-bold" : "border-transparent text-[#4a6550] hover:text-[#1a5c38]"}`}
                  >
                    {opt.label}
                  </button>
                ))}
          ...
          <button
            ...
            onClick={runSimulatePrediction}
            disabled={isSimulating}
            className="w-full py-3 bg-[#1a5c38] text-white hover:bg-[#134429] font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer disabled:opacity-40"
          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className={`p-4 rounded-xl border shadow-sm ${getDecisionTag(prediction.decision).color}`}>
                  ...
                  <div className="flex items-center gap-1.5 mt-2.5">
                    {React.createElement(getDecisionTag(prediction.decision).icon, { className: "w-5 h-5" })}
                    <p className="text-xs font-bold tracking-wide">
                      {getDecisionTag(prediction.decision).text}
                    </p>
                  </div>
                  <p className="text-[10px] text-gray-500 mt-1 relative flex items-center gap-1">
                    <span>Forecast Trend:</span>
                    <span className={`font-bold ${prediction.trend === "UP" ? "text-green-600" : prediction.trend === "DOWN" ? "text-red-700" : "text-amber-600"}`}>
                      {prediction.trend}
                    </span>
                  </p>
                </div>
              ...
              <div className="p-5 rounded-2xl border border-[#d1e4d5] bg-white space-y-4 shadow-sm">
                <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-2">
                  <div className="space-y-0.5">
                    <h4 className="text-sm font-bold text-[#1a5c38] tracking-wider uppercase flex items-center gap-1.5">
                      <ChartIcon className="w-4 h-4 text-[#1a5c38]" /> Price Trajectory Simulation
                    </h4>
                    <p className="text-xs text-[#7a9a80]">
                      Proj. Modal Price vs Expected Min/Max corridor widths for {prediction.cropName} ({prediction.market})
                    </p>
                  </div>
                  
                  <span className="px-2.5 py-1 text-[10px] bg-[#f4f8f4] border border-[#d1e4d5] text-[#4a6550] rounded-lg self-start">
                    Mandi ID: {prediction.cropName.replace(/\s+/g, "")}-{timeframe}d
                  </span>
                </div>
                ...
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={prediction.dataPoints}>
                      ...
                      <XAxis dataKey="label" stroke="#7a9a80" fontSize={10} tickLine={false} />
                      ...
                      <Line 
                        ...
                        stroke="#1a5c38" 
                        strokeWidth={2.5} 
                        dot={{ fill: "#fff", stroke: "#1a5c38", strokeWidth: 2, r: 4 }} 
                ...
                <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 text-center pt-2">
                  {prediction.dataPoints.map((pt) => (
                    <div key={pt.label} className="p-2 rounded-lg bg-[#f4f8f4] border border-[#d1e4d5] space-y-0.5 shadow-sm">
                      <span className="text-[10px] font-bold text-[#1a5c38] block">{pt.label}</span>
                      <span className="text-[10px] text-[#7a9a80] block truncate">{pt.date}</span>
                      <span className="text-xs font-bold text-[#1a2e1c] block">₹{pt.predictedPrice}</span>
                      <span className="text-[9px] text-[#7a9a80] block font-semibold">Acc: {pt.confidenceScore}%</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-6 rounded-2xl border border-[#d1e4d5] bg-white relative overflow-hidden shadow-sm">
                ...
                <div className="flex justify-between items-center border-b border-[#e8f2e9] pb-3 mb-4">
                  <h4 className="text-sm font-bold uppercase tracking-wider text-[#1a2e1c] flex items-center gap-1.5">
                    <BookOpen className="w-4 h-4 text-[#1a5c38]" /> Agronomical & Price Forecasting Advisory
                  </h4>
                  <span className="text-[10px] font-bold text-[#1a5c38] bg-[#edf4ee] px-2 py-0.5 rounded border border-[#d1e4d5]">
                ...
                <div className="mt-5 pt-4 border-t border-[#e8f2e9] flex flex-wrap gap-4 text-xs text-[#7a9a80]">
                  ...
                  <span className="flex items-center gap-1.5">
                    <Thermometer className="w-3.5 h-3.5 text-[#1a5c38]" /> Soil Carbon Status: <span className="font-semibold text-[#4a6550]">{soilFactor.toUpperCase()}</span>
                  </span>
                  <span className="flex items-center gap-1.5">
          ...
          {!prediction && !isSimulating && (
            <div className="p-8 rounded-2xl border border-[#d1e4d5] bg-white flex flex-col justify-center items-center py-24 text-center space-y-3.5 min-h-[460px] shadow-sm">
              ...
              <p className="text-xs text-[#7a9a80] max-w-sm leading-relaxed">
                Configure your target farming conditions on the Left parameter cards and trigger the ML solver code to render 180-days commodity rates index trajectories.
              </p>
              <button
                onClick={runSimulatePrediction}
                className="py-2.5 px-5 bg-[#1a5c38] text-white font-bold rounded-xl text-xs hover:bg-[#134429] cursor-pointer transition-colors shadow-sm"
              >
